...
# 绘制平滑后的曲线，并添加阴影
line1 = plt.fill_between(group1_daily_income['day'], group1_daily_income['smoothed'], color='tab:blue', alpha=0.2)  # 阴影
line1_plot = sns.lineplot(x='day', y='smoothed', data=group1_daily_income, color='tab:blue', linewidth=2)

line2 = plt.fill_between(group2_daily_income['day'], group2_daily_income['smoothed'], color='tab:green', alpha=0.2)  # 阴影
line2_plot = sns.lineplot(x='day', y='smoothed', data=group2_daily_income, color='tab:green', linewidth=2)

line3 = plt.fill_between(group3_daily_income['day'], group3_daily_income['smoothed'], color='tab:red', alpha=0.2)  # 阴影
line3_plot = sns.lineplot(x='day', y='smoothed', data=group3_daily_income, color='tab:red', linewidth=2)

# 添加图例到原图形

handles = [plt.Rectangle((0, 0), 1, 1, color='tab:blue', alpha=0.2),
           plt.Rectangle((0, 0), 1, 1, color='tab:green', alpha=0.2),
           plt.Rectangle((0, 0), 1, 1, color='tab:red', alpha=0.2)]

plt.xlabel('days',fontsize=20)
plt.ylabel('average individual income',fontsize=20)
# 调整刻度字体大小
plt.xticks(fontsize=18)  # 调整 x 轴刻度字体大小
plt.yticks(fontsize=18)  # 调整 y 轴刻度字体大小
# 限制 x 轴和 y 轴范围
plt.xlim(3, 28)  # 限制 x 轴范围
plt.ylim(200, group1_daily_income['smoothed'].max() * 1.1)  # 限制 y 轴范围，y 轴最大值设为平滑曲线最大值的 1.1 倍
